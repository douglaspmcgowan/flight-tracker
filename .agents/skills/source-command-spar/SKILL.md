...
Before seeding the loop, check the exact project path `.agents/work/state.json`. When it exists, the structured state is authoritative and generated `PROJECT.md`, `TRACKS.md`, `TASK.md`, `BACKBURNER.md`, and `LOG.md` views are read-only.
...
- Add or update reproduced-defect work in the current cell through `Update-WorkState.ps1`; use `Test-WorkScopeGuard.ps1` before writes and `Invoke-WorkScopeEvidence.ps1` for executed regression evidence.
- Route unreproduced, adjacent, deferred, or out-of-cell findings through `Capture-WorkDiscovery.ps1`. Use `New-WorkHandoff.ps1` for distinct ownership.
- A present but invalid state fails closed. Never edit generated views or fall back to legacy files.

References to direct `TASK.md` edits below apply only to unenrolled legacy projects.
